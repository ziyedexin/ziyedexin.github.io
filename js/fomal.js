...
function createtime1() {
  var grt = new Date("03/03/2022 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by 子夜的星 %c 你正在访问子夜の博客",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 子夜的星") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('子夜的博客站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var start = new Date("03/03/2022 00:00:00"); // 旅行者1号开始计算的时间
...
if (window.localStorage.getItem("fpson") == undefined || window.localStorage.getItem("fpson") == "1") {
  ...
  var loop = function () {
    ...
    if (now > 1000 + lastTime) {
      ...
      if (fps <= 5) {
        var kd = `<span style="color:#bd0000">网络太卡了🤢</span>`
      } else if (fps <= 15) {
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://zydx-520.oss-cn-beijing.aliyuncs.com/img/sa.jpg);
    --darkmode-bg:url(https://zydx-520.oss-cn-beijing.aliyuncs.com/img/bj.png);
    --mobileday-bg: url(https://zydx-520.oss-cn-beijing.aliyuncs.com/img/sa.jpg);
    --mobilenight-bg: url(https://zydx-520.oss-cn-beijing.aliyuncs.com/img/sa.jpg);
  }`;
}
// 切换背景主函数
function changeBg(s) {
